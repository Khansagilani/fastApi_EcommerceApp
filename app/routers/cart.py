...
@router.post("/items")
async def add_to_cart(item: CartItemCreate, session: SessionDep, current_user: User = Depends(get_current_user)):
...
async def remove_from_cart(cart_item_id: int, session: SessionDep, current_user: User = Depends(get_current_user)):
